...
// Função para encontrar um JWT (string que começa com "ey") em qualquer lugar de um objeto
function findJwt(obj) {
    for (const key in obj) {
        if (typeof obj[key] === 'string' && obj[key].startsWith('ey')) {
            return obj[key];
        }
...
export default async function handler(req, res) {
  ...
  try {
    const apiResponse = await fetch(PREZAO_API_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-USER-ID': phoneNumber, 'X-PINCODE': otpCode, 'X-CHANNEL': 'WEB' },
    ...
    // Procura o token em toda a resposta
